...
# 一次扫描 skills/*/SKILL.md，建 uuid→路径 与 名字→路径 双映射（角色 meta 按 uuid 引用技能）
function Get-SkillMap([string]$SkillsRoot) {
    $uuidMap = @{}; $nameMap = @{}
    if (-not (Test-Path $SkillsRoot)) { return @($uuidMap, $nameMap) }
    foreach ($d in Get-ChildItem $SkillsRoot -Directory) {
        ...
        if ($content -match '(?ms)^---\r?\n(.*?)\r?\n---') {
            $front = $Matches[1]
            if ($front -match '(?m)^uuid:\s*([0-9a-f]{8}-[0-9a-f-]+)\s*$') { $uuidMap[$Matches[1]] = $f }
            if ($front -match '(?m)^name:\s*(.*)$') {
                $nm = $Matches[1].Trim()
                if ($nm) { $nameMap[$nm] = $f }
            }
        }
    }
    return @($uuidMap, $nameMap)
}

# 根据技能 uuid 数组生成技能段（全局技能库；缺失 uuid 优雅跳过，不出现该技能）
function Get-RoleSkillsText {
    param([string]$RoleName, [string[]]$Skills)
    if (-not $Skills) { return '' }
    $maps = Get-SkillMap -SkillsRoot (Join-Path $configDir 'skills')
    $uuidMap = $maps[0]; $nameMap = $maps[1]
    $lines = @('## 特化技能', '你掌握以下技能：')
    $found = 0
    foreach ($sk in $Skills) {
        if ([string]::IsNullOrWhiteSpace($sk)) { continue }
        # 兼容：uuid 走 uuidMap；旧名字（迁移前 meta）走 nameMap
        if ($sk -match '^[0-9a-f]{8}-[0-9a-f-]+$') { $path = $uuidMap[$sk] }
        else { $path = $nameMap[$sk] }
        if (-not $path) { continue }            # 优雅跳过：未导入/已删除技能不出现在技能段
        $desc = Get-SkillDescription $path
        if ([string]::IsNullOrWhiteSpace($desc)) { $desc = '（无描述）' }
        $skillName = (Split-Path (Split-Path $path -Parent) -Leaf)  # 目录名 = 当前技能名
        $lines += "- **$skillName**（$desc）：相关任务时调用该技能。"
        $found++
    }
